...
users_admin = ["jsmith", "BB"]
users_vet = ["rbriggs", "AA"]
...
def fetch_all_patients(db):
    ...
    # patients = pd.read_sql_query(select_patients_query, db)
...
def search_appointment(db):
    search_option = st.selectbox(
        "Select search option", ["ID", "Patient ID", "Doctor Name"], key="search_option"
    )
...
def update_patient_record(db):
    ...
    search_option = st.selectbox(
        "Select search option", ["ID", "Contact Number", "CNIS"], key="search_option"
    )
    search_value = st.text_input("Enter search value", key="search_value")
...
def edit_patient(db):
    ...
    if st.button("Update :roller_coaster:"):
        ...
        update_patient_info(
            db, patient_id, new_name, new_age, new_contact, new_email, new_address
        )
...
def vet_access():
    ...
    menu = ["Home", "Add patient Record", "Show patiet Records"]
...
def admin_access():
    ...
    menu = [
        ...
        "Deetel Patients Record",
        "Add patients Appointments",
        "Show All Appointments",
        "Search and Edit Patients Appointments",
    ]
    options = st.sidebar.radio("Select an Option :dart:", menu)
    if options == "Home":
        st.subheader("Welcome to Hospital Mnagement System")
